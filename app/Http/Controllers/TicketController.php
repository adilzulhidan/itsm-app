    ...
    public function index(Request $request)
{
    ...
    public function show(Ticket $ticket)
    {
        ...
        if (in_array($user->role, ['admin', 'it_head'])) {
            $isAuthorized = true;
        } elseif ($user->role == 'manager' && ($ticket->user_id == $user->id || $ticket->department == $user->department)) {
            $isAuthorized = true;
    ...
    public function approveManager(Request $request, Ticket $ticket)
    {
        ...
        if (Auth::user()->department != $ticket->department) {
            return back()->with('error', 'Bukan departemen Anda.');
        }
        ...
        try {
            $itHead = User::where('role', 'it_head')->first();
            if ($itHead) {
                Mail::to($itHead->email)->send(new ITHeadApprovalNotification($ticket));
            }
        } catch (\Exception $e) {
            \Log::error('Email error: ' . $e->getMessage());
        }
    ...
    {
        if (!in_array(Auth::user()->role, ['it_head', 'admin'])) {
            return back()->with('error', 'Akses Ditolak');
        }
